// Admit One Ticket — vendored from 21st.dev.
//
//   https://21st.dev/@larsen66/components/admit-one-ticket   (id 22433)
//   npx shadcn@latest add "https://21st.dev/r/larsen66/admit-one-ticket"
//
// Retrieved through the 21st.dev MCP `get_component` and dropped in unmodified:
// it is the author's published build, plain JS against react/jsx-runtime with no
// shadcn `cn`, no framer-motion and no other dependency, so it needed no port.
// A WebGL dithered gradient does the ticket face, with a clip-path stub, notches
// and perforation, a tilt on pointer move and a glare sweep.
//
// Everything visual is passed in — `style` (texture + gradient colours),
// `layout` (ink and watermark) and `geometry` — so it is themed at the call site
// in BookingElements rather than edited here. Keep this file as delivered so it
// can be diffed against a future version from the registry.
